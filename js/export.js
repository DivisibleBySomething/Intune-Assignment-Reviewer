export function exportDashboard(apps, allGroups) {
  const html = generateExportHTML(apps, allGroups);
...
function generateExportHTML(apps, allGroups) {
  // Embed both datasets; the inline script has no ES-module imports
...
<style>
:root{--primary:#0078d4;--bg:#f3f2f1;--card-bg:#fff;--border:#e0e0e0;--text:#323130;--text-muted:#605e5c;--radius:6px;--shadow:0 2px 8px rgba(0,0,0,.1)}
...
.btn-sm{display:inline-flex;align-items:center;padding:4px 10px;border:1px solid var(--border);border-radius:4px;font-size:12px;color:var(--text);background:var(--card-bg);white-space:nowrap;text-decoration:none;transition:background .15s}
...
/* ── Group stats (mirrors buildGroupStats in pages.js) ───────────────────── */
...
function renderCharts() {
  ...
  new Chart(document.getElementById('chart-intents'), {
    ...
    data: {
      ...
      datasets: [{ label: 'Assignments', data: [intentCounts.required, intentCounts.available, intentCounts.uninstall, intentCounts.other], backgroundColor: ['#d13438','#107c10','#605e5c','#8a8886'], borderRadius: 4 }]
    },
  ...
  new Chart(document.getElementById('chart-groups'), {
    type: 'bar',
    data: {
      labels: top10.map(function(x) { return x[0]; }),
      datasets: [{ label: 'Assignments', data: top10.map(function(x) { return x[1]; }), backgroundColor: '#0078d4', borderRadius: 4 }]
    },
...
function renderAppPills() {
  ...
  document.getElementById('apps-filter-pills').innerHTML = APP_FILTERS.map(function(f) {
    return '<button class="filter-pill' + (f.id === currentAppFilter ? ' active' : '') + '" data-filter="' + f.id + '">' +
      esc(f.label) + '<span class="filter-pill-count">' + counts[f.id] + '</span></button>';
  }).join('');
...
function renderAppsTable() {
  var apps     = EMBEDDED_DATA.apps;
  var filtered = filterApps(apps, currentAppFilter);
  if (appSearchQuery) {
  ...
  tbody.innerHTML = filtered.map(function(app) {
    var statusBadge = app.isAssigned
    ...
    var intentBadges = app.assignments.length
      ? Object.entries(ic).filter(function(e) { return e[1] > 0; })
          .map(function(e) { return renderBadge(e[0]) + ' <span class="intent-count">' + e[1] + '</span>'; }).join(' ')
    ...
    return '<tr>' +
      '<td><div class="t-name">' + esc(app.appName) + '</div>' +
          '<div class="t-sub">' + esc(app.appType) + (app.publisher ? ' \xB7 ' + esc(app.publisher) : '') + '</div></td>' +
      '<td>' + statusBadge + '</td>' +
      '<td>' + intentBadges + '</td>' +
      '<td>' + groupCount + '</td>' +
...
function renderGroupPills() {
  ...
  document.getElementById('groups-filter-pills').innerHTML = GROUP_FILTERS.map(function(f) {
    return '<button class="filter-pill' + (f.id === currentGroupFilter ? ' active' : '') + '" data-filter="' + f.id + '">' +
      esc(f.label) + '<span class="filter-pill-count">' + counts[f.id] + '</span></button>';
  }).join('');
...
function renderGroupsTable() {
  var groups   = getGroupStats();
  var filtered = filterGroups(groups, currentGroupFilter);
  if (groupSearchQuery) {
  ...
  tbody.innerHTML = filtered.map(function(g) {
    var nameCell = g.isBuiltIn
      ? esc(g.name) + ' <span class="badge badge-available" style="font-size:10px">Built-in</span>'
      : esc(g.name);

    var statusBadge = g.total === 0
      ? '<span class="badge badge-uninstall">Empty</span>'
      : '<span class="badge badge-available">' + g.total + ' app' + (g.total !== 1 ? 's' : '') + '</span>';

    var openBtn = g.isBuiltIn
      ? '<span style="color:var(--text-muted);font-size:12px">Built-in</span>'
      : '<a href="https://intune.microsoft.com/#view/Microsoft_AAD_IAM/GroupDetailsMenuBlade/~/Overview/groupId/' + encodeURIComponent(g.id) + '" target="_blank" rel="noopener" class="btn-sm">Open &#x2197;</a>';
...
var EXPORT_CHANGELOG = [
  {
    ...
    items: [
      ...
      'Offline export now includes section tabs, filter pills, All Groups table, and this What\u2019s New modal',
    ],
  },
  {
